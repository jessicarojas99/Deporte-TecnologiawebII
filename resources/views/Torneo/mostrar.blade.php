...
<div class="container"> 
    <div class="col-sm-10  mx-auto bg-white shadow rouned">
        <h1 class=" display-4 text-center">Torneos</h1>
        <h4 class="text-center">registrados</h4>
        <hr style="background-color:red">
            <div class="card-body d-flex justify-content-between ">
                @auth
                <a class="btn btn-outline-dark btn-lg float-left" href="{{route('torneo.create')}}">
                   Añadir torneo
                </a>
                @endauth
                <form class="form-inline my-2 my-lg-0" method="GET" action="{{route('torneo.search')}}" autocomplete="off">                                     
                ...
                <table class="table table-striped"> 
                    ...
                    <tbody>
                        @forelse ($torneo as $torneoitem)
                        <tr>
                            <td> {{$torneoitem->id}}</td>
                            <td> {{$torneoitem->name}}</td>
          ...
          <div class="modal-dialog">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <form id="delete" action="{{route('torneo.destroy',$torneoitem)}}" method="POST">
                  @csrf  @method('DELETE')
